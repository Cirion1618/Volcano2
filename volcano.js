...
function eventsVolcano(state) {
    if (state == true) {
        ...
        displayOnCoolingToggleVolcano.addEventListener("click", displayOnCoolingToggleVolcanoHandler);
        ...
        try {
            ...
            characteristicCurrTempV.removeEventListener("characteristicvaluechanged", handleCurrTemperatureChangedVolcano);
    ...
    if (!isBleProcessPendingWithoutAlert_Repeat()) {
        ...
        characteristicPrj1V.readValue().then(value => {
            ...
            if (characteristicPrj1Value & MASK_PRJSTAT1_VOLCANO_ERR || characteristicPrj2Value & MASK_PRJSTAT2_VOLCANO_ERR) {
                ...
                var str = "";
                if (characteristicPrj2Value & MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING)
                    str = str + "- Temperature display during cool down phase disabled. Activate function if required.\n";
                if (characteristicPrj3Value & MASK_PRJSTAT3_VOLCANO_VIBRATION)
                    str = str + "- Vibration when setpoint reached disabled. Activate function if required.\n";
                if (ledBrightness < 25)
                    str = str + "- Display brightness set to low value. Change brightness if required.\n";
                if (str.length > 1)
                    alert(str);
                else
                    alert("Device analysis successful and without any issues.")
            }
        }
        ).catch(error => {
            setBleProcessPending(false);
    ...
    if (calcCheckValue(uint8Buf) == uint8Buf[len - 2]) {
        ...
        if (strChar.startsWith("RV0 222 BL")) {
            ...
            if (strChar.includes(firmwareChecksumNew))
                characteristicBootloaderWrite.writeValue(sendCRChecksum(firmwareChecksumNew)).then(value => {
                    setBleProcessPending(false)
                }
                );
            else if (strChar.includes(firmwareChecksumOld))
                characteristicBootloaderWrite.writeValue(sendCRChecksum(firmwareChecksumOld)).then(value => {
                    setBleProcessPending(false)
                }
                );
            else {
...
function handleUARTBootloader(event) {
    ...
    if (parseTelegram(uint8BufTemp))
        tempBufUARTBootloader = new ArrayBuffer(0)
}
function disconnectHybrid() {
    if (whichDeviceConnected() == "Hybrid") {
        ...
        var i = 0;
        for (; i > elemente.length; i++)
            elemente[i].classList.add("d-none")
    }
}
function volcanoConnect() {
    ...
    var i = 0;
    for (; i < elemente.length; i++)
        elemente[i].classList.remove("d-none");
    ...
    bluetoothDevice.gatt.connect().then(server => {
        ...
        return characteristic.startNotifications().then(_ => {
            characteristic.addEventListener("characteristicvaluechanged", handleUARTBootloader)
        }
        )
    }
    ).then(_ => {
        ...
        if (typeof characteristicBootloaderWrite.writeValueWithoutResponse === "function")
        ...
        if (result.length < 3 || firmwareFileMajorHybrid != result[1] || !checkVersionIsGreater(firmwareFileMinorHybrid, result[2]) || browserSupportsWriteWithoutResponse == false) {
            ...
            element.innerHTML = "The firmware of your VOLCANO HYBRID is outdated. The update will restart your VOLCANO HYBRID: please reconnect to finish the firmware update." + "The device can not be used during the update and must stay connected to the power grid.";
        ...
        if ((currentVal & MASK_PRJSTAT3_VOLCANO_VIBRATION) == 0)
            ...
            vibrationToggleVolcano.checked = false;
        currentThenValue = 26
    }
    ).then(service => {
        ...
        if (deviceHasErrorsV == true)
            startAnalysisVolcanoFunc()
    }
    ).catch(error => {
        eventsVolcano(false);
...
function onPlusButtonMouseDownVolcano() {
    if (whichDeviceConnected() == "Hybrid") {
        ...
        if (isCelsiusActive() == true)
            setSollTemperatureVisible(SOLLTEMP + 1);
        else
            setSollTemperatureVisible(SOLLTEMP + 1 / 1.8);
...
function showCurrentTemperature(temperature) {
    ...
    if (isCelsiusActive() == true)
        istTempLabel.innerText = Math.round(currentTemperatureVolcano).toString() + "\u00b0C";
    else
        istTempLabel.innerText = Math.round(convertToFahrenheitFromCelsius(currentTemperatureVolcano)).toString() + "\u00b0F"
}
...
function clickOnTempLabelClickVolcano() {
    if (whichDeviceConnected() == "Hybrid")
        if (isCelsiusActive() == true)
            ...
            projectRegister2ChangeBit(MASK_PRJSTAT2_VOLCANO_FAHRENHEIT_ENA)
}
function evalPrj1Volcano(currentVal) {
    ...
    if ((currentVal & MASK_PRJSTAT1_VOLCANO_ENABLE_AUTOBLESHUTDOWN) == 0)
        ...
        timerObject.classList.remove("d-none")
}
function evalPrj2Volcano(currentVal) {
    ...
    if ((currentVal & MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING) == 0)
        ...
        displayOnCoolingToggleVolcano.checked = false
}
...
function vibrationToggleVolcanoHandler() {
    if (!vibrationToggleVolcano.checked)
        ...
        projectRegister3ChangeBit(MASK_PRJSTAT3_VOLCANO_VIBRATION)
}
function displayOnCoolingToggleVolcanoHandler() {
    if (!isBleProcessPendingWithoutAlert_Repeat())
        if (displayOnCoolingToggleVolcano.checked)
            ...
            projectRegister2ChangeBit(65536 + MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING)
}
...
function setSollTemperatureVisible(temperature) {
    ...
    if (isCelsiusActive() == true)
        document.getElementById("sollTemp").innerText = Math.round(SOLLTEMP).toString() + "\u00b0C";
    else
        document.getElementById("sollTemp").innerText = convertToFahrenheitFromCelsius(SOLLTEMP).toString() + "\u00b0F"
}
;
