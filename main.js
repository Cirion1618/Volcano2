...
const boostSuperBoostTimeoutToggle = document.getElementById("boostModeTimeout");
...
if (userAgent_iOS()) {
    ...
    if (window.navigator.userAgent.includes("WebBLE/1")) {
...
boostSuperBoostTimeoutToggle.addEventListener("click", boostSuperBoostModeTimeoutToggleHandler);
...
function setUsedTimeOfDevice(useHoursValue, useMinutes) {
    if (useMinutes != 0)
        ...
        useHoursLabel.innerText = useHoursValue + "h"
}
...
function onPlusButtonMouseDown() {
    ...
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap") {
        ...
        if (isCelsius == true)
            setShowSollTempMobile(SOLLTEMP + 1);
        else
            setShowSollTempMobile(SOLLTEMP + 1 / 1.8);
...
function onPlusButtonMouseUp() {
    ...
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (timerPlusButtonMouseDown != null)
...
function onMinusButtonMouseUp() {
    ...
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (timerMinusButtonMouseDown != null)
...
function onMinusButtonBoostMouseDown() {
    if (myTimerBoostHandler !== 0)
        window.clearTimeout(myTimerBoostHandler);
    ...
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap") {
        ...
        myTimerBoostHandler = window.setTimeout(myTimerBoost, timeInMilliSeconds);
        if (isCelsius == true)
            setShowBoostTempMobile(BOOSTTEMP - 1);
        else
            setShowBoostTempMobile(BOOSTTEMP - 1 / 1.8);
...
function onMinusButtonSuperBoostMouseDown() {
    ...
    if (whichDeviceConnected() == "Qvap") {
        ...
        myTimerSuperBoostHandler = window.setTimeout(myTimerSuperBoost, timeInMilliSeconds);
        if (isCelsius == true)
            setShowSuperBoostTempMobile(SUPERBOOSTTEMP - 1);
        else
            setShowSuperBoostTempMobile(SUPERBOOSTTEMP - 1 / 1.8);
...
function updateCurrTemperatureMobile(temperature) {
    ...
    if (isCelsius == true)
        ...
        istTempLabel.innerText = convertToFahrenheitFromCelsius(temperature).toString() + "\u00b0F"
}
...
function setShowBoostTempMobile(boostTemperature) {
    ...
    if (whichDeviceConnected() == "Qvap")
        ;
    else if (SOLLTEMP + BOOSTTEMP > 210)
        BOOSTTEMP = 210 - SOLLTEMP;
    if (isCelsius == true)
...
function setShowSuperBoostTempMobile(superboostTemperature) {
    ...
    if (whichDeviceConnected() == "Qvap")
        ;if (isCelsius == true)
        superboostTempLabel.innerText = Math.round(SUPERBOOSTTEMP).toString() + "\u00b0C";
    else {
...
function myTimerSetBoostSuperBoostMobile() {
    ...
    if (window.getComputedStyle(sollTempLabel).visibility === "hidden")
        ...
        sollTempLabel.style.visibility = "hidden";
    animateBoostSuperBoostMobile()
}
function animateBoostSuperBoostMobile() {
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (superBoostModeActive)
            myTimerSetBoostSuperBoostHandlerMobile = window.setTimeout(myTimerSetBoostSuperBoostMobile, 500);
        else if (boostModeActive)
            myTimerSetBoostSuperBoostHandlerMobile = window.setTimeout(myTimerSetBoostSuperBoostMobile, 1E3);
...
function setShowSollTempMobile(temperature) {
    ...
    if (whichDeviceConnected() == "Qvap")
        ...
        tempSOLLTEMP = limitValueToRange(SOLLTEMP + BOOSTTEMP + superBoostVal, 40, 210);
    else if (boostModeActive)
        tempSOLLTEMP = limitValueToRange(SOLLTEMP + BOOSTTEMP, 40, 210);
    if (isCelsius == true)
        sollTempLabel.innerText = Math.round(tempSOLLTEMP).toString() + "\u00b0C";
    else
        sollTempLabel.innerText = convertToFahrenheitFromCelsius(tempSOLLTEMP).toString() + "\u00b0F";
    setShowBoostTempMobile(BOOSTTEMP)
}
...
function setBleProcessPending(status) {
    if (status === true)
        myTimerBLEpendingHandler = window.setTimeout(myTimerBLEpendingTimeoutFunc, 2E3);
...
function isBleProcessPendingWithoutAlert_Repeat() {
    if (bleInProcess == true) {
        ...
        case 1:
            ...
            break;
        case 2:
            window.setTimeout(window[callerFunctionName], 100, arguments[0], arguments[1]);
            break;
        default:
...
function onButtonClick() {
    ...
    if (navigator.bluetooth) {
            ...
            navigator.bluetooth.requestDevice(options).then(device => {
                ...
                if (whichDeviceConnected() == "Hybrid")
                    ...
                    if (!isConnectBrowser())
                        ...
                        setSerialNumber(bluetoothDevice.deviceName.split(" ")[1]);
                    QvapConnect()
                } else
                    craftyConnect();
                ...
                if (error.toString().includes("User cancelled") || error == 2)
                    ...
                    alert("Bluetooth connection error GENERAL: please reload and retry.\n" + error.toString() + "\n" + error.stack + "\n" + currentThenValue)
            }
            )
        } catch (error) {
            showError(error)
        ...
        if (iOS_BLEnotWorking())
            ...
            alert("Web Bluetooth is not supported by this browser. Please use another browser.");
        return 0
    }
}
...
function userAgent_iOS() {
    if (window.navigator.userAgent.includes("iPhone") || window.navigator.userAgent.includes("iPad") || window.navigator.userAgent.includes("WebBLE/1"))
        return true;
    else
        return false
}
...
function whichDeviceConnected() {
    if (typeof bluetoothDevice.deviceName != "undefined")
        connectBrowser = true;
    if (typeof bluetoothDevice.name != "undefined" && bluetoothDevice.name.includes("S&B VOLCANO") || typeof bluetoothDevice.deviceName != "undefined" && bluetoothDevice.deviceName.includes("S&B VOLCANO"))
        return "Hybrid";
    else if ((typeof bluetoothDevice.name != "undefined" && bluetoothDevice.name.includes("S&B VY") || typeof bluetoothDevice.deviceName != "undefined" && bluetoothDevice.deviceName.includes("S&B VY")) || (typeof bluetoothDevice.name != "undefined" && bluetoothDevice.name.includes("S&B VZ") || typeof bluetoothDevice.deviceName != "undefined" && bluetoothDevice.deviceName.includes("S&B VZ")))
        return "Qvap";
    else
        return "Crafty"
}
